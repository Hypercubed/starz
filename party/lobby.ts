...
export class LobbyServer extends Server {
  ...
  constructor(readonly room: DurableObjectState, readonly env: any) {
    super(room, env);
  }

  async onStart() {
    // await this.room.storage.put<LeaderboardMap>(STORAGE_LEADERBOARD_KEY, new Map<string, LeaderboardItem>());
    this.leaderboard = await this.room.storage.get<LeaderboardMap>(STORAGE_LEADERBOARD_KEY) ?? new Map<string, LeaderboardItem>();
  }

  async saveLeaderboard() {
    await this.room.storage.put<LeaderboardMap>(STORAGE_LEADERBOARD_KEY, this.leaderboard ?? new Map<string, LeaderboardItem>());
  }
  ...
  async onRequest(req: Request) {
    ...
    return new Response(JSON.stringify({ message: "Hello from LobbyServer!" }), {
      status: 200,
      headers: {
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Headers': 'Content-Type, Authorization'
  ...
  private onRequestScore(req: Request) {
    ...
    if (!playerId) {
      return new Response(JSON.stringify({ error: "Bad Request: Missing playerId" }), { status: 400, headers: HEADERS });
    }

    const playerEntry = this.leaderboard.get(playerId) as LeaderboardEntry | undefined;
