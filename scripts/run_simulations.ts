...
async function main() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const winner = result.winner
      ? `${result.winner.id} (${result.winner.name})`
      : 'None';
    console.log(
      `Game ${result.gameId}: Winner=${winner}, Ticks=${result.ticks}`
    );
    results.push(result);
  }
  ...
  Object.entries(wins).forEach(([player, count]) => {
    ...
    console.log(
      `Player ${player} (${name}): ${count} wins. Ticks: Min=${min}, Max=${max}, Avg=${avg.toFixed(0)}`
    );
  });
  console.log('==========================\n');
}
