...
import type { Coordinates, GameState, Lane, System } from '../game/types';
...
export function drawMap(el: HTMLElement, ctx: FnContext) {
  ...
  function resizeCanvas() {
    drawCanvas(globalThis.gameManager.getState());
  }
}
...
export function centerOnHome() {
  ...
  const home = getPlayersHomeworld(state);
  if (!home) return;
...
export function requestRerender() {
  ...
  function rerender(ctx?: FnContext) {
    if (!svg) return;
    ctx ??= globalThis.gameManager.getFnContext();
